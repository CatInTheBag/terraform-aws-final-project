...
resource "aws_instance" "myapp-ec2" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "./config.sh --url https://github.com/CatInTheBag/terraform-aws-final-project --token ${var.runner_token} --labels self-hosted,ubuntu,ec2 --name aws-ec2 --unattended --replace aws-ec2",
      "./run.cmd &"
    ]
  }
